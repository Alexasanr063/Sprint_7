...
class TestCourierDeletion:
    ...
    def test_delete_nonexistent_courier(self):
        response = requests.delete(Accept_Order.remove_courier_1)
        assert response.status_code == 404

    def test_delete_with_empty_id(self):
        response = requests.delete(Accept_Order.remove_courier_2)
        assert response.status_code == 404

    def test_delete_existing_courier(self):
        response = requests.delete(Accept_Order.remove_courier_3)
        print(response.json())
