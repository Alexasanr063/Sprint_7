...
class TestOrderAcceptance:
    ...
    def test_nonexistent_order(self):
        ...
        response = requests.put(Accept_Order.Url_accept_order_1)
        assert response.json()['message'] == Text_Of_Mistake.errer_1

    def test_nonexistent_courier(self):
        """Попытка принять заказ несуществующим курьером"""
        response = requests.put(Accept_Order.Url_accept_order_2)
        assert response.json()['message'] == Text_Of_Mistake.errer_2

    def test_missing_parameters(self):
        """Попытка принять заказ без указания всех параметров"""
        response = requests.put(Accept_Order.Url_accept_order_3)
    ...
    def test_empty_courier_id(self):
        ...
        response = requests.put(Accept_Order.Url_accept_order_4)
        assert response.json()['message'] == Text_Of_Mistake.errer_3

    def test_successful_order_acceptance(self):
        """Успешное принятие заказа"""
        get_id_response = requests.get(Accept_Order.Url_accept_order_5)
