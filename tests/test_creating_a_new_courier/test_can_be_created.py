...
import random
import string
from Sprint_7.data import Accept_Order,Text_Of_Mistake,Create_Data_For_Use
...
class TestCourierRegistration:
    ...
    def test_register_new_courier_and_return_login_password(self):
        ...
        login_pass = []
        payload = Create_Data_For_Use.register_new_courier()
        response = requests.post(Accept_Order.can_be_created_courier, data=payload)
    ...
    def test_all_required_fields_no_login(self):
        ...
        response = requests.post(Accept_Order.can_be_created_courier, data=payload)
        assert response.json()['message'] == Text_Of_Mistake.errer_4
    ...
    def test_cannot_create_identical_couriers(self):
        ...
        response1 = requests.post(Accept_Order.can_be_created_courier, data=payload)

        # Второй запрос - пытаемся создать такого же
        response2 = requests.post(Accept_Order.can_be_created_courier, data=payload)
